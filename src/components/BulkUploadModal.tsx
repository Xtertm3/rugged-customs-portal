...
export const BulkUploadModal: React.FC<BulkUploadModalProps> = ({ onClose, onUpload, teamMembers, vendors, currentUser, onAddVendor, firebaseService }) => {
  ...
  const [mode, setMode] = useState<'legacy' | 'bulkCreate'>('legacy');
    ...
    return (
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in" onClick={onClose}>
        <div className="bg-zinc-800 border border-zinc-700 w-full max-w-lg rounded-2xl shadow-2xl p-8 transform transition-all duration-300" onClick={e => e.stopPropagation()}>
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-zinc-100">Bulk Upload</h2>
            <button onClick={onClose} className="text-zinc-400 text-2xl font-bold hover:text-white transition-colors">&times;</button>
          </div>
          <div className="flex gap-4 mb-6">
            <button className={`px-4 py-2 rounded-lg font-semibold ${mode === 'legacy' ? 'bg-blue-700 text-white' : 'bg-zinc-700 text-zinc-300'}`} onClick={() => setMode('legacy')}>Bulk Assign Activities (Legacy)</button>
            <button className={`px-4 py-2 rounded-lg font-semibold ${mode === 'bulkCreate' ? 'bg-blue-700 text-white' : 'bg-zinc-700 text-zinc-300'}`} onClick={() => setMode('bulkCreate')}>Bulk Create Sites (New)</button>
          </div>
          {!uploadResult ? (
            <>
              {mode === 'legacy' && (
                <>
                  <div className="mb-4 text-sm text-zinc-400 space-y-2">
                    ...
                    <p className="pt-2 text-xs text-zinc-500">The `siteName` must exactly match an existing site. The `stage` must be 'Civil' or 'Electricals'.</p>
                  </div>
                  <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-zinc-600 rounded-lg p-8 text-center cursor-pointer transition-all mb-4">
                  ...
                  <button onClick={handleUploadClick} disabled={!selectedFile || isProcessing} className="w-full flex justify-center items-center px-6 py-3 bg-blue-700 text-white font-semibold rounded-lg shadow-lg hover:bg-blue-800 disabled:bg-blue-600/50 disabled:cursor-not-allowed transition-all">
            ...
            <div>
              ...
              <div className="bg-zinc-900/50 p-4 rounded-md space-y-2">
                <p className="text-green-400">Successfully processed: <span className="font-bold">{uploadResult.success}</span></p>
              ...
              <button onClick={onClose} className="mt-6 w-full px-6 py-2 bg-zinc-600 text-white font-semibold rounded-lg hover:bg-zinc-700 transition-all">Close</button>
            </div>
          )}
        </div>
      </div>
